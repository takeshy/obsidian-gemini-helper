...
チャット履歴とワークフロー実行ログをパスワード保護。

> **必須:** 暗号化を使用するには、最初にプラグイン設定でパスワードを設定する必要があります。

![暗号化設定](setting_encryption.png)

**設定手順:**
1. プラグイン設定で暗号化を有効化
2. パスワードを設定（公開鍵暗号方式で安全に保存）
3. 新しいチャットファイルとワークフロー履歴が暗号化されます

**機能:**
- **自動暗号化** - 新規チャットとワークフローログは保存時に暗号化
