...
- **エージェントスキル** - チャットをスキルで拡張可能。Obsidian 用 Markdown、Canvas、Bases は組み込み済みで、Dashboard Hub は対応する Dashboard スキルを動的に提供
...
- **+ ボタン** - 新規チャット
- **履歴ボタン** - 過去のチャットを読み込み
- **拡大・縮小ボタン** - サイドバーを通常幅と広い閲覧幅で切り替え
- **ノートとして保存ボタン** - 現在の会話をコンパクトな Markdown として手動保存。同じチャットを再保存すると同じノートを上書き
...
- **コンテキストに応じたスキル** - Dashboard、Canvas、Base のいずれかのファイルが開いているとき、チャットは汎用の Markdown スキルの代わりに対応するスキルを自動的に使用します。Dashboard のコンテキストは Dashboard Hub が有効な場合に利用できます
...
ダッシュボード機能は、別プラグインの [Dashboard Hub](https://github.com/takeshy/obsidian-dashboard-hub) が提供します。両方のプラグインを有効にすると、Gemini Helper は Dashboard Hub に設定済みの Gemini モデル、チャット連携、Base 生成、テキスト書き換え、ワークフローの生成・実行機能を提供します。また、Dashboard Hub は `dashboard` エージェントスキルを実行時に Gemini Helper へ登録します。
...
- **チャット履歴を自動保存** - 復元可能なチャット履歴をワークスペースフォルダに保存
- **チャット履歴の最大保存件数** - 上限を超えた古い自動履歴を削除。既存ユーザーは `0`（無制限）、新規インストールは `100` が初期値
...
- **チャットの手動保存先** - Chat の「ノートとして保存」ボタンで使う Vault 相対フォルダ。空欄の場合は Vault 直下
- 手動保存は `YYYYMMDD-HHmmss_チャットタイトル.md` 形式で、frontmatter や復元用メタデータを含まないコンパクトな会話本文のみを出力

### 暗号化

チャット履歴、ワークフロー実行ログ、個別の暗号化ファイルを保護する暗号鍵を設定します。
...
各ログ設定は独立して有効/無効を切り替えできます。暗号化ファイルエディタに必要なのは最初のパスワード/鍵設定だけで、どちらのログ暗号化トグルも有効にする必要はありません。
...
MCP は HTTP 接続に対応しています。stdio コマンドの解析は対象外です。
