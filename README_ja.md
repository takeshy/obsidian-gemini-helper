...
> **外部スキルリポジトリのルールについては、[import_skill.md](docs/import_skill.md)を参照してください**
...
> **OKF の構造と制限については、[OKF.md](docs/OKF.md)を参照してください**
...
ウィジェットのレスポンシブグリッドから、自分専用の**ホーム/概要ページ**を構築できます。ダッシュボードは `.dashboard` ファイルで、**Bases ビュー**・**ノート**・**Web ページ**・**タイムライン**・**ワークフロー出力**・**カンバンボード**をドラッグ＆リサイズ可能なグリッドに配置します。通常のノートと同じように開くだけで、ライブで編集可能なボードが表示されます。
