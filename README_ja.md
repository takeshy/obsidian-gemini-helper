...
ボード定義は `Dashboards/Kanbans/` 配下の再利用可能な `.kanban` YAML ファイルとして保存されます。既存のインライン定義は自動的にそこへ移行されます。複数のダッシュボードから同じ定義を参照でき、1つのウィジェット設定で行った変更は共有ファイルに反映されます。カードの並び順はダッシュボードウィジェットごとに保持されます。ボードヘッダーには一時的なタグフィルターもあります。
