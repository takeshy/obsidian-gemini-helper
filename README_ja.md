...
> [!TIP]
> **🚀 Gemini CLI: Pro レベルのモデルを無料で使える！（実験的機能）**
>
> [Gemini CLI](https://github.com/google-gemini/gemini-cli) を使えば、Google アカウントだけで **Gemini 2.5 Pro**（最高性能モデル）が使えます！API キーも課金も不要。CLI をインストールして Google アカウントでログインするだけで、最高のモデルでチャットできます。
>
> **注意:** この機能は実験的であり、将来のバージョンで削除される可能性があります。

## 無料プラン vs 有料プラン vs Gemini CLI

| 機能 | 無料 API | 有料 API | 🚀 Gemini CLI |
|------|----------|----------|---------------|
| 基本チャット | ✅ | ✅ | ✅ |
| Vault 操作（Function Calling） | ✅（Gemini モデルのみ） | ✅ | 読み取り/検索のみ |
| Web 検索 | ✅ | ✅ | ❌ |
| セマンティック検索 | ✅（制限あり） | ✅ | ❌ |
| 画像生成 | ❌ | ✅ | ❌ |
| 利用可能モデル | Flash, Flash Lite, Gemma | Flash/Pro, 画像モデル | **Gemini 2.5 Pro** 🎉 |
| 必要なもの | 無料 API キー | 有料 API キー | **Google アカウントのみ** |
| レート制限 | 厳しい | 緩い | 寛大 |

**なぜ Gemini CLI？** Google アカウントだけで **Gemini 2.5 Pro**（通常は有料 API キーが必要なフラッグシップモデル）が使えます。API 費用なしで最高品質の回答を得たいユーザーに最適です。
...
- **HTML インフォグラフィック プレビュー** - HTML コードブロックをプレビュー・保存（AI 生成のインフォグラフィックに最適）。モーダルはドラッグ移動・リサイズ可能
...
### Gemini CLI 設定（実験的機能）
API キーの代わりに Gemini CLI を使用できます：
1. [Gemini CLI](https://github.com/google-gemini/gemini-cli) をインストール
2. ターミナルで `gemini` を実行し、`/auth` で認証を完了
3. プラグイン設定の CLI Settings で「Command line mode」を有効化
4. 「Verify」をクリックして CLI が動作することを確認
5. 検証完了後、チャットのモデル選択に「Gemini CLI (Experimental)」が表示されます

**動作要件：**
- **macOS/Linux:** `gemini` コマンドが PATH に含まれていること
- **Windows:** gemini-cli が `%APPDATA%\npm` にインストールされていること（npm グローバルインストールのデフォルト場所）

**CLI モードの制限事項：**
- Vault への書き込み操作は使用不可（読み取り・検索のみ）
- セマンティック検索は使用不可
- Web 検索は使用不可
...
#### セキュリティ (CLI モード)
- CLI モードは外部コマンド `gemini` (Google の Gemini CLI) を実行します
- コマンドインジェクション防止のため `shell: false` で実行
- ユーザー入力は引数として渡され、シェル展開されません
- モバイルデバイスでは CLI モードは完全に無効化されます
- 実行されるコマンド: `gemini --version` (検証) と `gemini -p "prompt"` (チャット) のみ
