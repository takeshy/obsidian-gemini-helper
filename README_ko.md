...
첨부 버튼 아래의 데이터베이스 아이콘(📦)을 통해 AI가 사용할 수 있는 Vault 도구를 제어합니다:
...
> **팁:** RAG 사용 시 "Vault: 검색 제외"를 권장합니다. RAG가 이미 Vault 전체에 대한 시맨틱 검색을 제공하므로 중복 검색을 피할 수 있습니다.
...
- **히스토리 보기** - 명령어: "Show edit history" 또는 명령 팔레트 사용
...
채팅 기록과 워크플로우 실행 로그를 비밀번호로 보호합니다.

> **필수:** 암호화를 사용하려면 먼저 플러그인 설정에서 비밀번호를 설정해야 합니다.

![암호화 설정](setting_encryption.png)

**설정 방법:**
1. 플러그인 설정에서 암호화 활성화
2. 비밀번호 설정 (공개키 암호화 방식으로 안전하게 저장)
3. 모든 새 채팅 파일과 워크플로우 기록이 암호화됩니다

**기능:**
- **자동 암호화** - 새 채팅과 워크플로우 로그가 저장 시 암호화
