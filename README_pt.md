...
- **Agent Skills** - Skills reutilizáveis expandem o chat; os skills Markdown, Canvas, Bases e Dashboard para Obsidian vêm integrados
...
- **Criptografia e Secret Manager** - Criptografe o histórico de chat e os logs de workflows e gerencie segredos criptografados no dashboard
- **Histórico de Edições** - Rastreie e restaure alterações feitas pela IA com visualização de diff
- **Painel** - Organize visualizações de Bases, arquivos, memos de leitura, páginas web, timelines, quadros Kanban e a saída de workflows em uma grade de widgets responsiva
...
## Entrada por voz e leitura em voz alta

**Ler as respostas** — Ative *Ler respostas automaticamente* no menu de ferramentas do Vault. Enquanto estiver ativo, um chip acima da entrada indica isso e o seu ✕ desativa. Cada balão de resposta também tem um botão de alto-falante para lê-la sob demanda ou interrompê-la no meio da frase. A velocidade da leitura pode ser ajustada logo abaixo do interruptor e em Configurações → Chat (de 0,5x a 5x; o quanto acelera de fato depende da voz: as vozes do Windows costumam parar de acelerar acima de cerca de 4x). Enquanto a leitura está ativa, o prompt do sistema pede ao modelo uma resposta falada e curta em vez de Markdown, títulos, listas, código ou URLs.

**Enviar o texto ditado** — Ative *Enviar automaticamente o texto ditado* nas configurações para enviar quando o texto colado ou ditado terminar com a frase de envio (`enviar` em português, editável por idioma). Funciona com o ditado do sistema, o Aqua Voice e ferramentas semelhantes.

**Conversa por voz** — Instale o [speech-popup](https://github.com/takeshy/speech-popup) e um botão de microfone aparece acima do botão de enviar. Ele apenas abre a janela, então uma janela fechada fica a um clique. Durante uma conversa:

- Fale e envie pela janela. O speech-popup marca o que cola com `⟦voice-chat⟧` (`show --append`), então o texto é enviado como sua mensagem; uma colagem comum da área de transferência permanece no campo de texto, assim como uma que chega enquanto uma resposta ainda está sendo gerada.
- A resposta é lida em voz alta (ativada junto com a conversa) e a janela reabre cerca de 2,5 segundos depois que a leitura termina, para que a sua própria voz sintetizada não seja gravada.
- Desativar a leitura pelo chip dela mantém a conversa: a janela continua reabrindo após cada resposta, em silêncio.
- **Para encerrar**: diga a frase de envio sem ditar mais nada (`I'm done speaking` por padrão). O speech-popup fecha como sempre, o chat recebe um turno vazio e o lê como "terminei", sem nenhum clique. Pressionar Enter com a janela vazia, o ✕ do chip e abrir outro chat também encerram.
...
- No Obsidian em Flatpak, autorize o portal do host uma vez com `flatpak override --user --talk-name=org.freedesktop.Flatpak md.obsidian.Obsidian` e informe acima o caminho completo do executável no host (por exemplo `/home/you/.local/bin/speech-popup`); os comandos passam por `flatpak-spawn --host`. O Snap não tem um portal equivalente: use o AppImage ou o Flatpak.
...
> [!NOTE]
> Menções de arquivos do vault com @ inserem apenas o caminho do arquivo — a IA lê o conteúdo via ferramentas.
...
> **Nota:** RAG, Web Search, ferramentas do Vault e MCP podem ser todos usados simultaneamente via a API de Interactions.

### Acesso da IA a pastas

Em **Configurações → Workspace → Pastas que a IA pode acessar automaticamente**, você pode escolher opcionalmente quais pastas a IA pode ler automaticamente. Use isso quando houver diretórios que você não quer que a IA leia, a menos que você anexe ou referencie arquivos explicitamente.

- Deixe vazio para permitir que a IA acesse todo o vault por operações automáticas do Vault
- Insira pastas relativas ao vault, como `Public` ou `Shared/Docs`, separadas por vírgulas
- O acesso à pasta raiz não é especificado com `/`, `.` ou um caminho vazio. Vazio significa sem restrição de pasta, permitindo o vault inteiro
- Segmentos de caminho `.` e `..` são rejeitados
- Esta configuração limita operações automáticas do Vault no Chat e em workflows de comando de IA. Ela não limita recuperação RAG, anexos manuais, ferramentas MCP, scripts ou nós workflow note diretos
- Para arquivos que a IA nunca deve ler, use criptografia em vez de depender apenas do escopo de pastas
...
- **Built-ins sensíveis ao contexto** - Quando um arquivo Dashboard, Canvas ou Base está aberto, o chat usa automaticamente o skill integrado correspondente em vez do skill Markdown genérico
...
- **Recuperação de erros de workflow** - Se um workflow de skill falhar durante um chat, a chamada de ferramenta com falha mostra um botão **Abrir workflow** que abre o arquivo *e* alterna a visão do Gemini para a aba Workflow / skill para que você possa editar e reexecutar imediatamente
...
> **Para instruções de configuração e exemplos, pergunte ao chat do Gemini Helper. A fonte de conhecimento OKF integrada contém a referência atual de Agent Skills.**
...
O Gemini Helper também vem com um pacote OKF integrado sobre seus próprios recursos. O chat pode usar esse conhecimento integrado para responder perguntas sobre configuração do plugin, ferramentas de chat, skills, workflows, RAG, OKF, MCP, painéis, segurança e solução de problemas, sem exigir um diretório OKF separado.

![Exemplo de OKF](docs/images/okf_sample.png)

> **Para a estrutura e os limites do OKF, pergunte ao chat do Gemini Helper ou navegue pelo pacote OKF em inglês em [`docs/okf/gemini-helper-help/`](docs/okf/gemini-helper-help/index.md).**
...
**Da Barra Lateral:**
1. Abra a aba **Workflow / skill** na barra lateral
...
1. Abra a aba **Workflow / skill** na barra lateral do Gemini
...
6. A IA produz primeiro um **plano** em linguagem simples — revise e clique em **OK** para prosseguir, **Replanejar** para dar feedback e regenerar o plano, ou **Cancelar**
7. Após a geração, a IA executa uma **revisão** do resultado. Se forem encontrados problemas, você pode escolher **OK** (com diálogo de confirmação), **Refinar** (regenerar usando o feedback da revisão) ou **Cancelar**. Revisões limpas avançam automaticamente
8. O workflow é salvo após você aceitar a pré-visualização final
...
Ao abrir a aba Workflow / skill com um arquivo que não tem bloco de código workflow, um botão **"Create workflow with AI"** é exibido. Clique para gerar um novo workflow (saída padrão: `workflows/{{name}}.md`).
...
4. O mesmo fluxo de plano → geração → revisão é executado. Você pode **Refinar** o resultado da revisão quantas vezes quiser; cada Refinar dispara uma nova passagem de geração e uma revisão nova para que a revisão exibida sempre corresponda ao YAML final
5. Revise o diff antes/depois
6. Clique em **Apply Changes** para atualizar

**Modificar Skill com IA:**

Quando o arquivo ativo é um `SKILL.md`, a aba Workflow / skill mostra um botão **"Modificar skill com IA"** em vez de (ou junto com) o modificador de workflow regular. Ele edita o skill como um todo — tanto o corpo de instruções do SKILL.md *quanto* o arquivo de workflow referenciado — em uma única passagem, preservando o frontmatter do skill (name, description, entradas de workflows).
...
Abra a aba **Workflow / skill** na barra lateral do Gemini para executá-lo.
...
> **Para especificações detalhadas de nós e exemplos, pergunte ao chat do Gemini Helper. A fonte de conhecimento OKF integrada contém a referência atual de workflows.**
...
Crie uma **página inicial / visão geral** pessoal a partir de uma grade responsiva de widgets. Um painel é um arquivo `.dashboard` que organiza **visualizações de Bases**, **arquivos**, **memos de leitura**, **páginas web**, **timelines**, **saída de workflow** e **quadros Kanban** em uma grade que pode ser arrastada e redimensionada — abra-o como qualquer nota para ver um quadro ativo e editável.

![Painel](docs/images/dashboard.png)

**Criar um painel:**
- Comando: **"Gemini Helper: Criar painel"** — cria um novo quadro em `Dashboards/` e o abre
...
**Edição:** Os painéis não exigem mais um modo de edição separado. Arraste os widgets para movê-los, arraste a alça no canto inferior direito para redimensionar, use a engrenagem em cada widget para configurá-lo e use **+ Adicionar widget** na barra de ferramentas para adicionar mais. Cada widget também tem um botão de maximizar; clicar nele mostra apenas aquele widget, e o ícone de restaurar volta para a grade normal. Todas as alterações são salvas automaticamente.

![Organizar widgets](docs/images/dashboard_arrange.gif)

## Tipos de widget

Clique em **+ Adicionar widget** para escolher um tipo:
...
| **File** | Um arquivo do vault renderizado inline: Markdown/texto/HTML, imagens, PDF, EPUB e outros arquivos com um botão para abrir | `path`, `showHeader` |
...
| **Kanban** | Notas como cartoes arrastaveis agrupados em colunas de status | arquivo `.kanban` opcional, filtro `tag`/`folder`, `statusProperty`, `columns`, `displayFields` |
| **Secret Manager** | Cria, pesquisa, visualiza, edita e copia segredos criptografados do vault | pasta opcional com arquivos `.encrypted` |
...
| **MemoList** | Um índice dos arquivos de memo de leitura do widget File em `Dashboards/Memos/` | nenhuma |

O widget **File** é a superfície de leitura para notas e documentos. Arquivos Markdown/texto/HTML são renderizados inline, imagens são renderizadas diretamente, e PDFs/EPUBs usam visualizações de leitura contínua. Selecione o texto e clique com o botão direito para **Copiar**, **Perguntar à IA** ou **Adicionar ao memo**. Os memos são armazenados junto com os dados do painel em `Dashboards/Memos/`, mantêm links de volta ao texto citado quando possível, e podem ser editados ou excluídos no painel de memos. Clicar em uma citação de memo salta de volta para o local de origem; enquanto o painel de memos está aberto, os trechos com memos são destacados no documento.

![Memo do widget File](docs/images/dashboard_memo.gif)
...
Transforme notas em um quadro de arrastar e soltar. Os cartoes sao notas que correspondem a um filtro de **tag** e/ou **pasta**, agrupados em colunas por uma **propriedade de status** do frontmatter. Arraste um cartao para outra coluna para atualizar o status dessa nota — gravado diretamente no frontmatter da nota. O quadro é totalmente interativo diretamente no painel.
...
- **Ordem manual** — arraste cartoes para cima/baixo dentro de uma coluna para salvar uma ordem personalizada para o quadro.
...
Configure tudo nas configuracoes do widget:
...
> [!NOTE]
> **Os widgets de workflow leem de um cache, nao ao vivo.** Um widget de workflow so e executado pelo botao **Executar**, pela execucao de teste do editor de configuracao, ou uma vez ao abrir quando seu resultado em cache e mais antigo que o **intervalo de atualizacao automatica** (minutos; `0` = somente manual). Os resultados sao armazenados como arquivos normais do vault em `Dashboards/Data/<encoded dashboard path>.json`, portanto sincronizam/versionam como outros arquivos e entram nos workflows de push/pull. O workflow deve armazenar sua saida Markdown/HTML em uma variavel (padrao `result`).

> **Para o formato do arquivo `.dashboard`, o esquema YAML e dicas de geração com IA, pergunte ao chat do Gemini Helper. A fonte de conhecimento OKF integrada contém a referência atual de painéis.**
...
1. Abra **Configurações → Community plugins** no Obsidian
2. Selecione **Browse**
3. Pesquise por `Gemini Helper`
4. Instale e ative o plugin

### BRAT
Use o BRAT se quiser testar versões beta diretamente deste repositório.
...
- **Pastas que a IA pode acessar automaticamente** - Lista opcional de pastas permitidas para acesso automático da IA ao vault. Deixe vazio para permitir todo o vault; use criptografia para arquivos que a IA nunca deve ler

![Limites de Ferramentas](docs/images/setting_tool_history.png)

### Criptografia

Configure as chaves que protegem o histórico de chat, os logs de workflows, arquivos criptografados individuais e o Secret Manager.
...
Cada configuração de log pode ser ativada ou desativada separadamente. O editor de arquivos criptografados e o Secret Manager exigem apenas a configuração inicial de senha e chaves.
...
- **Metadados pesquisáveis** - Adicione uma descrição e metadados chave/valor explicitamente públicos sem descriptografar o arquivo
...
> **Dica:** Para criptografar todos os arquivos em um diretório de uma vez, use um workflow. Pergunte ao chat do Gemini Helper sobre o exemplo atual de workflow `obsidian-command`.
...
> **Desde a v1.11.0, este plugin foca exclusivamente em funcionalidades relacionadas ao Gemini.**
> O suporte a CLI foi removido. Um novo plugin [obsidian-llm-hub](https://github.com/takeshy/obsidian-llm-hub) foi criado com suporte a CLI e múltiplos provedores de LLM (OpenAI, Claude, OpenRouter, Local LLM).

| Plugin | Descrição |
|--------|-----------|
| obsidian-gemini-helper | Focado no Gemini (RAG via File Search API) |
| obsidian-llm-hub | Suporte multi-LLM, apenas Desktop (RAG via Embedding, suporta gemini-embedding-2-preview) |
| obsidian-local-llm-hub | Apenas LLM local (RAG apenas via embeddings locais) |
