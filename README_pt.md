...
- No Linux, um Obsidian em Flatpak roda isolado e não enxerga programas instalados no host, então os comandos passam automaticamente por `flatpak-spawn --host`. Autorize uma vez com `flatpak override --user --talk-name=org.freedesktop.Flatpak md.obsidian.Obsidian`. A versão Snap não tem um portal equivalente: use o AppImage ou o Flatpak.
