...
**朗讀回覆** — 在 Vault 工具選單中開啟「自動朗讀回覆」。開啟時輸入框上方會顯示標籤，點選其 ✕ 即可關閉。每則回覆氣泡也有喇叭按鈕，可隨時朗讀該則回覆或中途停止。朗讀速度可在開關正下方調整，也可在設定（設定 → 聊天）中調整（0.5 倍到 5 倍；實際能加速到多少取決於語音，Windows 的語音通常在約 4 倍以上就不再變快）。朗讀開啟時，系統提示會要求模型以簡短口語回答，而非 Markdown、標題、清單、程式碼或網址。

**自動送出語音輸入** — 在設定中開啟「自動送出語音輸入」，當貼上或輸入的文字以送出短語（中文預設為 `發送`，可依語言修改）結尾時自動送出。可搭配系統聽寫、Aqua Voice 等工具使用。

**語音對話** — 安裝 [speech-popup](https://github.com/takeshy/speech-popup) 後，送出按鈕上方會出現麥克風按鈕。它一律只是開啟彈出視窗，因此關掉的視窗按一次就會回來。對話進行中：

- 說完後從彈出視窗送出，內容會直接作為提問送出。speech-popup 會為它貼上的內容加上 `⟦voice-chat⟧` 標記（`show --append`），因此一般的剪貼簿貼上不會被送出，而是留在輸入框中；在回覆產生期間送達的內容同樣留在輸入框，不會遺失。
- 回覆會被朗讀（對話開始時會自動開啟朗讀），朗讀結束約 2.5 秒後彈出視窗會再次開啟，避免麥克風錄到自己的朗讀聲。
- 以朗讀標籤的 ✕ 只關閉朗讀時，對話仍會繼續：每次回覆後視窗依舊開啟，只是不再朗讀。
- **結束方式**：不說其他內容，只說送出短語（預設 `I'm done speaking`）即可。speech-popup 照常關閉，聊天收到一個空回合並視為「我說完了」，完全不需點選。在空的彈出視窗中按 Enter、點選標籤的 ✕、或開啟另一個聊天同樣可以結束。
- 結束後彈出視窗仍保持開啟。之後它貼上的內容只會去掉標記，文字進入輸入框而不送出——需要分幾次口述長文再自己送出時，就這樣用。
- 若 Obsidian 的 PATH 找不到該應用程式，請在設定 → 聊天 →「speech-popup 命令」中填入完整路徑。失敗時會連同命令與錯誤內容一起提示。
- 在 Flatpak 版 Obsidian 上，先以 `flatpak override --user --talk-name=org.freedesktop.Flatpak md.obsidian.Obsidian` 授權一次主機通道，然後在上方的命令欄填入主機上的完整路徑（例如 `/home/you/.local/bin/speech-popup`）；命令會透過 `flatpak-spawn --host` 執行。Snap 版沒有對應的通道，請改用 AppImage 或 Flatpak 版。

[在 YouTube 上觀看語音對話示範](https://www.youtube.com/watch?v=w8uyateB-0s)
...
用響應式小工具網格建立你專屬的**首頁/概覽頁**。儀表板是一個 `.dashboard` 檔案，它將 **Bases 檢視**、**筆記**、**網頁**、**時間軸**、**工作流輸出**和**看板**排列在可拖曳和調整大小的網格中——像開啟任何筆記一樣開啟它，即可看到可即時編輯的面板。
...
看板定義會儲存為 `Dashboards/Kanbans/` 下可重複使用的 `.kanban` YAML 檔案。既有的內嵌定義會自動遷移到該處。多個儀表板可共用同一定義；卡片順序仍由每個儀表板小工具個別保存。
...
- 可在詳細對話框中編輯值、說明和公開中繼資料；檔案會保持加密。
- 明文值只在解鎖時存在於記憶體中，絕不會以未加密形式儲存到 Vault。

![編輯 Secret Manager](docs/images/secret_manager_edit.png)
