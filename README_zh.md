...
- **Agent Skills** - 可用技能扩展聊天；内置了 Obsidian Markdown、Canvas、Bases 和 Dashboard 技能
...
**朗读回复** — 在 Vault 工具菜单中打开「自动朗读回复」。开启时输入框上方会显示一个标签，点击其 ✕ 即可关闭。每条回复气泡上也有喇叭按钮，可随时朗读该条回复或中途停止。朗读速度可在开关正下方调整，也可在设置（设置 → 聊天）中调整（0.5 倍到 5 倍；实际能加速到多少取决于语音，Windows 的语音通常在约 4 倍以上就不再变快）。朗读开启时，系统提示会要求模型用简短的口语作答，而不是 Markdown、标题、列表、代码或网址。

**自动发送语音输入** — 在设置中打开「自动发送语音输入」，当粘贴或输入的文本以发送短语（中文默认为 `发送`，可按语言修改）结尾时自动发送。可配合系统听写、Aqua Voice 等工具使用。

**语音对话** — 安装 [speech-popup](https://github.com/takeshy/speech-popup) 后，发送按钮上方会出现麦克风按钮。它始终只是打开弹窗，因此关掉的弹窗一按即回。对话进行时：

- 说完后从弹窗发送，内容会直接作为提问发送。speech-popup 会为它粘贴的内容加上 `⟦voice-chat⟧` 标记（`show --append`），因此普通的剪贴板粘贴不会被发送，而是留在输入框中；在回复生成期间到达的内容同样留在输入框，不会丢失。
- 回复会被朗读（对话开始时会自动开启朗读），朗读结束约 2.5 秒后弹窗再次打开，避免麦克风录到自己的朗读声。
- 用朗读标签的 ✕ 只关闭朗读时，对话仍会继续：每次回复后弹窗依旧打开，只是不再朗读。
...
- 在 Flatpak 版 Obsidian 上，先用 `flatpak override --user --talk-name=org.freedesktop.Flatpak md.obsidian.Obsidian` 授权一次宿主机通道，然后在上面的命令中填写宿主机上的完整路径（例如 `/home/you/.local/bin/speech-popup`）；命令会通过 `flatpak-spawn --host` 执行。Snap 版没有对应的通道，请改用 AppImage 或 Flatpak 版。
...
- **情境感知内置技能** - 当打开 Dashboard、Canvas 或 Base 文件时，聊天会自动使用匹配的内置技能，而不是通用的 Markdown 技能
...
> **有关设置说明和示例，请咨询 Gemini Helper 聊天。内置的 OKF 知识来源包含当前的 Agent Skills 参考文档。**
...
Gemini Helper 还内置了一个介绍自身功能的 OKF 捆绑包。聊天可以使用这些内置知识回答有关插件设置、聊天工具、技能、工作流、RAG、OKF、MCP、仪表板、安全性和故障排除的问题，而无需单独配置 OKF 目录。

![OKF 示例](docs/images/okf_sample.png)

> **有关 OKF 结构和限制，请咨询 Gemini Helper 聊天，或浏览英文 OKF 捆绑包 [`docs/okf/gemini-helper-help/`](docs/okf/gemini-helper-help/index.md)。**
...
> **有关详细的节点规范和示例，请咨询 Gemini Helper 聊天。内置的 OKF 知识来源包含当前的工作流参考文档。**
...
用响应式小组件网格构建你的个人**主页/概览页**。仪表板是一个 `.dashboard` 文件，它将 **Bases 视图**、**文件**、**阅读批注**、**网页**、**时间线**、**工作流输出**和**看板**排列在可拖拽和调整大小的网格中——像打开任何笔记一样打开它，即可看到实时可编辑的面板。

![仪表板](docs/images/dashboard.png)

**创建仪表板：**
- 命令：**“Gemini Helper: 创建仪表板”** — 在 `Dashboards/` 下创建一个新面板并打开它
- 或在聊天中请求 AI（内置的 **dashboard** 智能体技能会为你创建 `.dashboard` 文件以及作为其基础的 `.base` 文件）

**编辑：** 仪表板不再需要单独的编辑模式。拖动小组件即可移动，拖动右下角的手柄可调整大小，点击每个小组件上的齿轮图标进行设置，并使用工具栏中的 **+ 添加小组件** 添加更多小组件。每个小组件还有一个最大化按钮；点击后只显示该小组件，点击还原图标即可返回正常网格视图。所有编辑都会自动保存。

![排列小组件](docs/images/dashboard_arrange.gif)
...
**File** 小组件是笔记和文档的阅读界面。Markdown/文本/HTML 文件会内联渲染，图片会直接显示，PDF/EPUB 则使用连续阅读视图。选中文本后右键可选择**复制**、**询问 AI** 或**添加到批注**。批注与仪表板数据一起存储在 `Dashboards/Memos/` 下，会尽可能保留指向引用文本的链接，并可以在批注面板中编辑或删除。点击某条批注引用会跳转回原文位置；批注面板打开时，文档中会高亮显示对应的批注范围。

![File 小组件批注](docs/images/dashboard_memo.gif)

**MemoList** 小组件列出整个仪表板中的批注文件。点击某一行不会跳转离开当前页面，而是将该小组件最大化，并打开所选文件及其批注面板。还原小组件即可返回 MemoList。

![MemoList 小组件](docs/images/memolist.png)
...
将笔记变成拖放式看板。卡片是匹配 **标签** 和/或 **文件夹** 筛选的笔记，按 frontmatter 中的 **状态属性** 分组到各列。将卡片拖到另一列即可更新该笔记的状态——直接写回笔记的 frontmatter。看板可直接在仪表板上完全交互。

![看板](docs/images/dashboard_kanban.png)

看板定义会保存为 `Dashboards/Kanbans/` 下可复用的 `.kanban` YAML 文件。已有的内联定义会自动迁移到那里。多个仪表板可以共享同一定义；卡片顺序仍由每个仪表板小组件单独保存。
...
- 可在详情对话框中编辑值、说明和公开元数据；文件会保持加密。
- 明文值仅在解锁时存在于内存中，绝不会以未加密形式保存到仓库。

![编辑 Secret Manager](docs/images/secret_manager_edit.png)
...
> **有关 `.dashboard` 文件格式、YAML 架构以及 AI 生成提示，请咨询 Gemini Helper 聊天。内置的 OKF 知识来源包含当前的仪表板参考文档。**
...
> **Thinking 模式：** 对于支持的 Gemini 模型，可在模型选择器旁边的下拉框中选择思考级别（`default` / `minimal` / `low` / `medium` / `high`）。`default` 表示交由 Gemini API 决定。**Gemini 3.1 Pro** 无法禁用 Thinking，因此其最低级别为 `low`；Gemma 4 和图像模型没有思考选择器。该设置按模型记忆，消息中的关键词不再自动更改思考级别。
...
> **提示：** 要一次性加密目录中的所有文件，请使用工作流。请咨询 Gemini Helper 聊天获取当前的 `obsidian-command` 工作流示例。
