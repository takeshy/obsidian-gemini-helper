...
ウィジェットのレスポンシブグリッドから、自分専用の**ホーム/概要ページ**を構築できます。ダッシュボードは `.dashboard` ファイルで、**Bases ビュー**・**ファイル**・**読書メモ**・**Web ページ**・**タイムライン**・**カンバンボード**・**ワークフロー出力**をドラッグ＆リサイズ可能なグリッドに配置します。通常のノートと同じように開くだけで、ライブで編集可能なボードが表示されます。
...
| | ダッシュボード | Canvas |
|---|-----------|--------|
| **コンテンツ** | **ライブ** — Bases ビュー、ファイル、読書メモ、タイムライン、カンバンボード、ワークフロー出力が自動的に更新されます | **静的** — カードは手動で配置するスナップショット |
...
| **操作** | 構造化されたグリッド上のウィジェット操作 | 常に編集可能 |
...
ダッシュボードに専用の編集トグルはありません。通常表示のまま編集できます:

- **ドラッグ / リサイズ** — ウィジェットをドラッグして移動し、右下のハンドルでサイズ変更します。
- **歯車** — ウィジェット設定を開きます。削除も設定パネル内から行います。
- **最大化 / 元に戻す** — 1 つのウィジェットだけを大きく表示し、アイコンで通常グリッドに戻します。
- **+ ウィジェットを追加** — ウィジェットパレットを開きます。
...
**+ ウィジェットを追加** をクリックしてウィジェットの種類を選びます:
...
同じ `.base` ファイルを複数の Base ウィジェットから参照できます。たとえば Active / Done / Backlog のように、ビューごとに 1 つのウィジェットを置けます。設定パネル外で `.base` ファイルが変更された場合、保存前に再読み込みするため、新しい内容を古い状態で上書きしません。

### File — Vault ファイルを読んでメモする

既存の Vault ファイルをインライン表示します。Markdown / text / HTML、画像、PDF、EPUB に対応し、それ以外のファイルは開くボタンを表示します。

![File ウィジェットの設定](images/dashboard_markdown.png)

| 設定 | 説明 |
|---------|-------------|
| **ファイル** | 埋め込む Vault ファイルのパス（検索可能なピッカー） |
| **ヘッダーを表示** | ファイルパス、開くボタン、メモボタンを含むヘッダーを表示 |

文書を読んでいるときは、文字列を選択して右クリックできます:

- **Copy** — 選択テキストをコピー。
- **AI に問い合わせ** — 選択テキストを Chat に入力済みの状態にする（自動送信はしません）。
- **メモに追加** — メモパネルを開き、選択範囲へのリンク付きでメモを作成。

メモは `Dashboards/Memos/` に、元ファイルのパスに対応するファイルとして保存されます。引用リンクには可能な範囲で前後文脈も保存されるため、同じ文字列が複数ある文書でもジャンプしやすくなります。メモパネルを開いている間、保存済みメモの範囲は本文上でハイライトされます。引用リンクがある場合は、コメント本文が空でもメモとして保存できます。
...
**タグ** および/または **フォルダ** フィルターに一致するノートを、フロントマターの **ステータスプロパティ** によって列にグループ化したカードとして表示します。カードを別の列にドラッグすると、そのノートのステータスが更新されます（`processFrontMatter` で書き込み）。 カードを列内で上下にドラッグすると、そのボード専用の手動順序が保存されます。カードをクリックするとモーダルでノートをプレビューでき、モーダルの遷移アイコンで新しいタブにノートを開きます。ボードはダッシュボード上で直接操作できます。
...
![Timeline インライン編集](images/timeline_edit.png)

composer とインライン編集欄のどちらからでも **AI で編集** を使えます。現在の下書き本文と指示をモデルに送り、生成されたリライトは差分で確認してから textarea に適用します。

![Timeline AI リライト](images/timeline_ai.png)

### MemoList — 読書メモを一覧する

`Dashboards/Memos/` に保存された File ウィジェットのメモファイルを一覧します。PDF、EPUB、Markdown ノートなどに付けた読書メモの入口として使えます。

行をクリックしてもダッシュボード外へ遷移しません。MemoList ウィジェットが最大化され、選択した元ファイルをメモパネル付きで一時表示します。最大化を解除すると MemoList の一覧に戻ります。
...
| ブレークポイント | 条件 | レイアウト |
|------------|------|--------|
| **`lg`**（広い） | ≥ 768px | 広いグリッド上で配置したレイアウト（既定 12 カラム） |
...
# file
config:
  path: Research/report.pdf        # supported file path in the vault
...
- **MemoList を読書メモの入口にする。** File ウィジェットのメモは `Dashboards/Memos/` に保存されます。MemoList は注釈付き文書へ戻るためのダッシュボード内インデックスになります。
- **デスクトップのみ。** ダッシュボード（プラグインの他の機能と同様）は Obsidian デスクトップで動作します。
- **ファイルは Vault 内にあります。** ダッシュボードは `Dashboards/` 配下に `.dashboard` ファイルとして、ワークフロー結果は `Dashboards/Data/`、読書メモは `Dashboards/Memos/`、タイムライン投稿は `Dashboards/Timeline/`、生成された Bases は `Dashboards/Bases/` に保存されます。これらは通常の Vault ファイルで、ノートと同様に同期/バージョン管理されます。
