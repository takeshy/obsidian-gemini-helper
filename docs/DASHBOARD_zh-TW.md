...
用響應式小工具網格建立你專屬的**首頁/概覽頁**。儀表板是一個 `.dashboard` 檔案，它將 **Bases 檢視**、**筆記**、**網頁**、**時間軸**、**看板**和**工作流輸出**排列在可拖曳和調整大小的網格中。像開啟任何筆記一樣開啟它，即可得到可即時編輯的面板。
...
| | 儀表板 | Canvas |
|---|-----------|--------|
| **內容** | **即時** — Bases 檢視、時間軸、看板、工作流輸出和筆記會自動更新 | **靜態** — 卡片是手動放置的快照 |
...
儀表板以普通 `.dashboard` 檔案儲存在你的 Vault 中，因此會像其他筆記一樣同步和版本管理。Workflow 小工具結果會作為普通 Vault 檔案另外儲存在 `Dashboards/Data/` 下。
...
- **檔案位於你的 Vault 中。** 儀表板以 `.dashboard` 檔案儲存在 `Dashboards/` 下，工作流結果在 `Dashboards/Data/`，時間軸貼文在 `Dashboards/Timeline/`，產生的 Bases 在 `Dashboards/Bases/`。它們都是一般 Vault 檔案，會與你的筆記一起同步/版本管理。
