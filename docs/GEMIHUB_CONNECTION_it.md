...
Sincronizza il tuo vault Obsidian con Google Drive, completamente compatibile con [GemiHub](https://gemihub.online). Modifica le note in Obsidian e accedi dall'interfaccia web di GemiHub, o viceversa.

## Cos'è GemiHub?

[GemiHub](https://gemihub.online) è un'applicazione web che trasforma Google Gemini in un assistente AI personale integrato con il tuo Google Drive.

![Interfaccia GemiHub](images/gemihub_connection/push_pull.png)

### Funzionalità esclusive di GemiHub

Queste funzionalità sono disponibili solo tramite l'interfaccia web di GemiHub e non possono essere replicate dal solo plugin Obsidian:

- **Automatic RAG** - I file sincronizzati su GemiHub vengono indicizzati automaticamente per la ricerca semantica. A differenza della sincronizzazione RAG manuale del plugin Obsidian, GemiHub indicizza i file ad ogni sincronizzazione senza configurazione aggiuntiva.
- **OAuth2-enabled MCP** - Usa server MCP che richiedono autenticazione OAuth2 (es. Google Calendar, Gmail, Google Docs). Il plugin Obsidian supporta solo l'autenticazione MCP basata su header.
- **Conversione Markdown in PDF/HTML** - Converti le tue note Markdown in documenti PDF o HTML formattati direttamente in GemiHub.
- **Pubblicazione pubblica** - Pubblica documenti HTML/PDF convertiti con un URL pubblico condivisibile, rendendo facile la condivisione delle note all'esterno.

### Funzionalità aggiunte a Obsidian tramite la connessione

Attivando la sincronizzazione Google Drive, queste funzionalità diventano disponibili lato Obsidian:
...
- **Risoluzione dei conflitti con diff** - Quando lo stesso file viene modificato su entrambi i lati, un modale di conflitto mostra un diff unificato con codice colore per aiutarti a decidere quale versione mantenere
- **Cronologia modifiche Drive** - Traccia le modifiche effettuate sia da Obsidian che da GemiHub, con voci di cronologia per file che mostrano l'origine (locale/remoto)
...
È necessario un account [GemiHub](https://gemihub.online) con la sincronizzazione Google Drive configurata. Il plugin utilizza il token di autenticazione crittografato di GemiHub per connettersi al tuo Google Drive.

1. Accedi a GemiHub
2. Vai su **Settings** → sezione **Obsidian Sync**
3. Copia il **Migration Tool token**
...
![Attivare la sincronizzazione Drive](images/gemihub_connection/setting_drive_sync.png)

3. Incolla il **Migration Tool token** da GemiHub
4. Clicca **Setup** per recuperare l'autenticazione crittografata da Google Drive

![Configurazione Migration Tool Token](images/gemihub_connection/setting_migration_tool_token.png)

5. Inserisci la tua **password** per sbloccare la sincronizzazione per la sessione corrente

![Sblocco Drive Sync](images/gemihub_connection/start_with_sync.png)
...
![Push su Drive](images/gemihub_connection/push.png)
...
![Pull in locale](images/gemihub_connection/pull_to_local.png)
...
![Risoluzione dei conflitti](images/gemihub_connection/conflict.png)

Clicca su **Diff** per vedere un diff unificato con codice colore tra le versioni locale e remota:

![Vista diff dei conflitti](images/gemihub_connection/conflict_diff.png)
...
![Backup dei conflitti](images/gemihub_connection/conflict_backup.png)
...
| **Migration Tool token** | Incolla dalle impostazioni di GemiHub (sezione Obsidian Sync) | - |
...
- Se hai cambiato la password in GemiHub, usa **Reset auth** nelle impostazioni e riconfigura con un nuovo Migration Tool token
