...
![GemiHub インターフェース](images/gemihub_connection/push_pull.png)

### GemiHub 限定機能

以下の機能は GemiHub の Web インターフェースでのみ利用可能で、Obsidian プラグイン単体では実現できません：

- **自動 RAG** - GemiHub に同期されたファイルは自動的にセマンティック検索用にインデックスされます。Obsidian プラグインの手動 RAG 同期とは異なり、GemiHub は同期のたびに追加セットアップなしでファイルをインデックスします。
- **OAuth2 対応 MCP** - OAuth2 認証が必要な MCP サーバーを使用可能（例：Google Calendar、Gmail、Google Docs）。Obsidian プラグインはヘッダーベースの MCP 認証のみをサポートしています。
- **Markdown から PDF/HTML への変換** - GemiHub 上で Markdown ノートをフォーマット済みの PDF または HTML ドキュメントに直接変換できます。
- **公開パブリッシュ** - 変換した HTML/PDF ドキュメントを共有可能な公開 URL で公開でき、外部へのノート共有が容易になります。

### 接続により Obsidian に追加される機能

Google Drive 同期を有効にすると、Obsidian 側で以下の機能が利用可能になります：

- **Diff プレビュー付き双方向同期** - 変更をコミットする前に、詳細なファイル一覧と Unified Diff ビューで Push・Pull を実行
- **Diff 付きコンフリクト解決** - 同じファイルが両方で編集された場合、カラーコード付き Unified Diff を表示するコンフリクトモーダルで、どちらのバージョンを保持するか判断できます
- **Drive 編集履歴** - Obsidian と GemiHub の両方での変更を追跡、ファイルごとの履歴エントリに変更元（ローカル/リモート）を表示
...
![Drive 同期を有効化](images/gemihub_connection/setting_drive_sync.png)

3. GemiHub からコピーした **Migration Tool token** を貼り付け
4. **Setup** をクリックして Google Drive から暗号化認証を取得

![Migration Tool Token の設定](images/gemihub_connection/setting_migration_tool_token.png)

5. **パスワード** を入力して現在のセッションの同期をアンロック

![Drive Sync のアンロック](images/gemihub_connection/start_with_sync.png)
...
![Drive への Push](images/gemihub_connection/push.png)
...
![ローカルへの Pull](images/gemihub_connection/pull_to_local.png)
...
![コンフリクト解決](images/gemihub_connection/conflict.png)

**Diff** をクリックすると、ローカルとリモートのバージョン間のカラーコード付き Unified Diff が表示されます：

![コンフリクト Diff ビュー](images/gemihub_connection/conflict_diff.png)
...
![コンフリクトバックアップ](images/gemihub_connection/conflict_backup.png)
...
| **Migration Tool token** | GemiHub の設定（Obsidian Sync セクション）から貼り付け | - |
...
- GemiHub でパスワードを変更した場合は、設定画面の **Reset auth** を使用し、新しい Migration Tool token で再セットアップしてください
