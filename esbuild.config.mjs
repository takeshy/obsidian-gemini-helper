...
const footer = `} catch (__loadError) {
  try {
    ...
    var __msg = "Gemini Helper failed to load: " + String(__loadError) + "\\n" + ((__loadError && __loadError.stack) || "");
    ...
    try {
      ...
      if (__app && __app.vault && __app.vault.adapter) { __app.vault.adapter.write("gemini-helper-load-error.log", __msg); }
    } catch (_) {}
  } catch (_) {}
  throw __loadError;
}`;
...
await buildChatStyles({ classPrefix: "gemini-helper" });
...
const context = await esbuild.context({
  ...
  plugins: [chatStylesPlugin({ classPrefix: "gemini-helper" }), ...(langfuseBuild ? [langfuseCompatPlugin] : [langfuseNoopPlugin])],
  loader: {
    ".ts": "ts",
    ".tsx": "tsx",
    ".css": "css",
