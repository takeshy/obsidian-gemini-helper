...
import { saveEditToDrive, ensureEditHistoryFolderId } from "./driveEditHistory";
...
export interface SyncFileListItem {
  ...
  type: "new" | "modified" | "deleted" | "editDeleted" | "renamed";
...
export class DriveSyncManager {
  ...
  /**
   * Initial setup: decode backup token, fetch encrypted auth from Drive, persist.
   */
  ...
  private findLocallyModifiedFiles(
  ...
  ): { modifiedIds: Set<string>; newPaths: Set<string>; renames: Map<string, string> } {
    ...
    return { modifiedIds, newPaths, renames };
  }
  ...
  async refreshSyncCounts(): Promise<void> {
    ...
    try {
      ...
      const vaultFiles = this.getAllVaultFiles();
      const { checksums } = await this.computeVaultChecksums(vaultFiles, localMeta);
      const { modifiedIds, newPaths, renames } = this.findLocallyModifiedFiles(localMeta, checksums);

      const diff = computeSyncDiff(localMeta, remoteMeta, modifiedIds);

      // Detect files tracked in meta but physically missing from disk
      const missingLocal = this.findMissingLocalFiles(localMeta, remoteMeta, checksums, diff, renames);
      diff.toPull.push(...missingLocal);

      // Push count = modified files + new files + renames
      this.localModifiedCount = diff.toPush.length + newPaths.size + renames.size + diff.editDeleteConflicts.length;

      // Pull count = remote changes + remote-only + locally-deleted-remotely
      if (!remoteMeta) {
        this.remoteModifiedCount = 0;
      } else {
        const pullLocalOnly = diff.localOnly.filter(id => id in localMeta.files);
        this.remoteModifiedCount = diff.toPull.length + diff.remoteOnly.length + pullLocalOnly.length + diff.editDeleteConflicts.length;
      }
    } catch (err) {
      console.debug("[DriveSync] Background count refresh failed:", err);
    }
    this.onStatusChange?.();
  }
  ...
  async computeSyncFileList(direction: "push" | "pull"): Promise<SyncFileListResult> {
    ...
    // Detect files tracked in meta but physically missing from disk (for pull)
    if (direction === "pull") {
      const missingLocal = this.findMissingLocalFiles(localMeta, remoteMeta, checksums, diff, renames);
      diff.toPull.push(...missingLocal);
    }
  ...
  async push(): Promise<void> {
    ...
    try {
      ...
      // 9. Handle localOnly files (exist in local meta but not remote = locally deleted)
      // Soft delete: move to trash/ subfolder instead of permanent deletion
      // Exclude renamed files (already handled above)
      const filesToTrash = diff.localOnly.filter(id => {
      ...
      if (historyEntries.length > 0) {
        ...
        (async () => {
          try {
            ...
            // best-effort: don't log errors for background history save
          }
        })();
      }
  ...
  async pull(): Promise<void> {
    ...
    try {
      ...
      const vaultFiles = this.getAllVaultFiles();
      const { checksums } = await this.computeVaultChecksums(vaultFiles, localMeta);
      const { modifiedIds, renames } = this.findLocallyModifiedFiles(localMeta, checksums);

      // 3. Compute diff
      const diff = computeSyncDiff(localMeta, remoteMeta, modifiedIds);

      // 3.1. Re-download files tracked in meta but physically missing from disk
      const missingLocal = this.findMissingLocalFiles(localMeta, remoteMeta, checksums, diff, renames);
      diff.toPull.push(...missingLocal);
  ...
  private async applyPullDiff(
    ...
    // 2. Download toPull + remoteOnly files
    ...
    // 3. Update local meta (collect fresh vault stats for mtime/size caching)
    const vaultStats = new Map<string, { mtime: number; size: number }>();
  ...
  async resolveConflict(fileId: string, choice: "local" | "remote"): Promise<void> {
    ...
    try {
      ...
      // If no more conflicts, continue pull after releasing the lock
    ...
    if (choice === "local") {
      ...
      if (file instanceof TFile) {
        if (isBinary) {
          ...
          await saveConflictBackup(accessToken, rootFolderId, `local_${vaultPath}`, localContent);
        } else {
  ...
  async restoreConflictFile(fileId: string, restoreName: string): Promise<void> {
    ...
    // Check if a file with the same name already exists in remote meta
    const remoteMeta = await readRemoteSyncMeta(accessToken, rootFolderId) ?? {
      lastUpdatedAt: new Date().toISOString(),
      files: {},
    };
    const existingId = Object.entries(remoteMeta.files)
      .find(([, f]) => (f.path ?? f.name) === restoreName)?.[0];

    // Read conflict file content and update or create
    let driveFile: drive.DriveFile;
    if (existingId) {
      // Update existing file to avoid duplicates
      if (isBinary) {
        const content = await drive.readFileRaw(accessToken, fileId);
        driveFile = await drive.updateFileBinary(accessToken, existingId, content, getMimeType(restoreName));
      } else {
        const content = await drive.readFile(accessToken, fileId);
        driveFile = await drive.updateFile(accessToken, existingId, content, getMimeType(restoreName));
      }
    } else {
      if (isBinary) {
        const content = await drive.readFileRaw(accessToken, fileId);
        driveFile = await drive.createFileBinary(accessToken, restoreName, content, rootFolderId, getMimeType(restoreName));
      } else {
        const content = await drive.readFile(accessToken, fileId);
        driveFile = await drive.createFile(accessToken, restoreName, content, rootFolderId, getMimeType(restoreName));
      }
    }

    // Update remote meta
    upsertFileInMeta(remoteMeta, driveFile, restoreName);
    await writeRemoteSyncMeta(accessToken, rootFolderId, remoteMeta);

    // Delete the conflict backup file
  ...
  async downloadTempToVault(tempFileId: string, payload: TempFilePayload): Promise<void> {
    ...
    // Write to vault
    if (payload.isBinary) {
      await this.app.vault.adapter.writeBinary(vaultPath, base64ToArrayBuffer(payload.content));
    } else {
      await this.app.vault.adapter.write(vaultPath, payload.content);
    }
