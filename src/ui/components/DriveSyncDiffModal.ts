...
export class DriveSyncDiffModal extends Modal {
  ...
  private renderFileItem(listEl: HTMLElement, file: SyncFileListItem): void {
    const itemCls = file.type === "conflict" ? "gemini-helper-sync-diff-file is-conflict" : "gemini-helper-sync-diff-file";
    ...
    switch (file.type) {
      ...
      case "conflict":
        iconName = "git-merge";
        iconCls = "gemini-helper-sync-diff-conflict";
        break;
    }
    ...
    if (file.type === "editDeleted") {
      ...
      const tagEl = headerEl.createSpan({ cls: "gemini-helper-sync-diff-tag gemini-helper-sync-diff-conflict-tag" });
