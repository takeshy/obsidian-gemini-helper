...
export function displayAgentPluginSettings(containerEl: HTMLElement, ctx: SettingsContext): void {
  ...
  containerEl.createDiv({ cls: "setting-item-description", text: `Install Agent Plugins v1.0.0 from a public GitHub repository. Packages are pinned to a commit and stored in ${agentPluginRoot()}. Gemini Helper supports plugin skills and streamable HTTP MCP servers.` });
  let repository = "";
  new Setting(containerEl).setName("GitHub repository").setDesc("Owner/repository or a GitHub URL")
    ...
    .addButton(button => button.setButtonText("Preview and install").setCta().onClick(() => { void (async () => {
      ...
      try {
        const preview = await previewAgentPlugin<McpServerConfig>(repository);
  ...
  for (const item of plugin.settings.agentPlugins) {
    ...
    setting.addToggle(toggle => toggle.setValue(item.enabled).setTooltip(item.enabled ? "Disable" : "Enable").onChange(value => { void (async () => {
      item.enabled = value;
      await plugin.app.vault.adapter.write(`${agentPluginRoot()}/${item.name}/install.json`, JSON.stringify(item, null, 2));
